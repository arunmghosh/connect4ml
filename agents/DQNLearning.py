...
class DQNLearning(Strategy):
    ...
    def learn(self, game, num_trials: int, is_training=True):
        ...
        policy_dqn = DQN(game.num_states, game.num_actions).to_device(device)
        if is_training:
            buffer = ReplayBuffer(self.replay_memory_size)
            epsilon = self.epsilon_init

        for t in range(num_trials):
            state = torch.tensor(game.board_state, device=device)
            game_log = []
